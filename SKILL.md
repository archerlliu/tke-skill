...
通过 `kubectl debug node` 在节点上创建临时特权 Pod，访问宿主机文件系统执行诊断命令，完成后自动清理 Pod。

> **前提条件**：集群 K8s 版本 >= 1.20，操作用户需有 `nodes/proxy` 和 `pods` 的创建权限。

#### node-debug shell - 进入节点交互式 shell

输出进入节点 shell 的命令（交互式操作需用户在终端手动执行）：

```bash
python {baseDirectory}/k8s_cli.py node-debug shell <node-name>
python {baseDirectory}/k8s_cli.py node-debug shell <node-name> -i ubuntu
```

#### node-debug dmesg - 查看节点内核日志

```bash
python {baseDirectory}/k8s_cli.py node-debug dmesg <node-name>
python {baseDirectory}/k8s_cli.py node-debug dmesg <node-name> --tail 100
```

#### node-debug kubelet-logs - 查看 kubelet 日志

```bash
python {baseDirectory}/k8s_cli.py node-debug kubelet-logs <node-name>
python {baseDirectory}/k8s_cli.py node-debug kubelet-logs <node-name> --tail 200
python {baseDirectory}/k8s_cli.py node-debug kubelet-logs <node-name> --since "1 hour ago"
```

#### node-debug crictl-status - 查看容器运行时状态

```bash
python {baseDirectory}/k8s_cli.py node-debug crictl-status <node-name>
# 包含已退出的容器
python {baseDirectory}/k8s_cli.py node-debug crictl-status <node-name> -a
```

#### node-debug atop - 获取 atop 日志文件

参数说明：
- `--date`：日志日期，格式 `YYYYMMDD`，默认今天
- `--atop-dir`：atop 日志目录，默认 `/var/log/atop`
- `-o`：保存到指定文件，默认保存到当前目录 `atop_<node>_<date>.log`

```bash
# 获取今天的 atop 日志
python {baseDirectory}/k8s_cli.py node-debug atop <node-name>
# 获取指定日期
python {baseDirectory}/k8s_cli.py node-debug atop <node-name> --date 20260408
# 指定 atop 目录和输出文件
python {baseDirectory}/k8s_cli.py node-debug atop <node-name> \
  --atop-dir /data/atop --date 20260408 -o ./node1_atop.log
```

#### node-debug diagnose - 一键诊断

汇总输出 dmesg、kubelet 日志、crictl 状态、磁盘和内存使用情况：

```bash
python {baseDirectory}/k8s_cli.py node-debug diagnose <node-name>
# 使用自定义 debug 镜像
python {baseDirectory}/k8s_cli.py node-debug diagnose <node-name> -i ubuntu
```

#### node-debug cleanup - 清理遗留 debug pod

```bash
python {baseDirectory}/k8s_cli.py node-debug cleanup
python {baseDirectory}/k8s_cli.py node-debug cleanup -n kube-system
```
